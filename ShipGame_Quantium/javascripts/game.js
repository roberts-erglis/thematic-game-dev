...
import { Ship, keepInSameHalf, clampYToHalf, transferAcrossHalves } from "./spaceship.js";
...
export class Game {
  ...
  initState() {
    ...
    this.mode  = "classic"; // classic / quantum
    this.gameOver = false;
 ...
 applyX() {
  ...
  applyH() {
    ...
    if (this.mode === "classic") {
      if (this.state === STATES.ZERO || this.state === STATES.ONE) {
        ...
        this.mode = "quantum";
        // create twin by straight transfer (same-from-top/midline rule)
        const twinYraw = transferAcrossHalves(this.ship.y);
        const twinHalf = (this.ship.y < BASE.HEIGHT) ? 1 : 0;
        this.twin = new Ship(this.ship.x, clampYToHalf(twinYraw, twinHalf), "#ffb05f");
        return true;
      }
      return false;
    } else {
  ...
  applyZ() {
    ...
    if (this.state === STATES.PLUS) { this.state = STATES.MINUS; return true; }
    if (this.state === STATES.MINUS){ this.state = STATES.PLUS;  return true; }
    return false;
  }

  // ========= Collisions & death/measurement =========
  rectsOverlap(a, b) {
  ...
  measureCollapse() {
    ...
    let destroyed = false;
    for (const e of this.enemies.items) {
      if (this.rectsOverlap(e.rect, this.ship.rect)) { destroyed = true; break; }
    }

    if (destroyed) {
      this.onDeath("Measurement Result: DESTROYED",
                   `Observed ${observed}. Your ship was destroyed upon collapse.`);
    } else {
      this.screen.showToast(`Measurement → observed ${observed}. Ship SURVIVED.`, 2200);
    }
  }
  ...
  maybeApplyAutoGate(now) {
    ...
    if (RANDOM.H_HITS.includes(r)) {
      changed = this.applyH(); applied = "H";
    } else if (RANDOM.X_HITS.includes(r)) {
      changed = this.applyX(); applied = "X";
    } else if (RANDOM.Z_HITS.includes(r)) {
      changed = this.applyZ(); applied = "Z";
    }
  ...
  update(dt, now) {
    if (this.gameOver) {
      if (this.input.hit("r") || this.input.hit("enter") || this.input.hit(" ")) this.initState();
      return;
    }
    ...
    if (this.mode === "classic") {
      ...
      this.ship.x = Math.max(12, Math.min(BASE.WIDTH - 12, this.ship.x + dx * dtScaled / (1/60))); // mild frame-normalization
      this.ship.y = keepInSameHalf(this.ship.y, this.ship.y + dy * dtScaled / (1/60));
    } else {
      if (this.input.down("arrowup"))   enemyVerticalDelta -= ENEMY.SPEED_Y_STEP;
      if (this.input.down("arrowdown")) enemyVerticalDelta += ENEMY.SPEED_Y_STEP;
